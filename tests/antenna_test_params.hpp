...
// ============================================================================
// antenna_test_params.hpp — параметры тестов антенной стратегии
//
// ЧТО:    AntennaTestParams (struct) + SignalVariant (enum) +
//         фабричные методы Small() / FullSpec() / Debug().
// ЗАЧЕМ:  Единый носитель всех параметров теста: размеры матриц, частоты,
//         задержки, вариант сигнала, опции сохранения — без дублирования
//         по всем тест-классам (GRASP Information Expert).
// ПОЧЕМУ: Параметры эталонного теста взяты из TestStrategia.md:
//         n_ant=2500, n_samples=5000, fs=0.5 МГц, n_beams=100.
//         Small()/Debug() дают быстрый прогон без ROCm на CI.
//
 ...
 * @brief AntennaTestParams и SignalVariant — параметры тестов стратегии.
